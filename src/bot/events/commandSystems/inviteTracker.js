...
function ready() {
    this.guilds.fetch(mainGuild).then(guild => guild.invites.fetch().then(invites => {
...
async function guildMemberAdd() {
    ...
    try {
        ...
        this.guild.channels.cache.get(loginoutChannel).send(`🟩 O usuário ${Formatters.userMention(this.user.id)} entrou através do código de convite \`${usedInvite.code}\`, gerado por ${Formatters.userMention(usedInvite.inviterId)}. Agora somos ${Formatters.bold(memberCount)}.`);
    } catch (err) {
        console.log(err);
    };
