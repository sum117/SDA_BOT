        ...
        if (msg.attachments.size >= 1) {
          msg.attachments.forEach((attachment) => {
            ...
            msg.guild.channels.cache
              .get(mediaChannel)
              .send({
                content: `Imagem enviada em ${Formatters.channelMention(
                  generalChannel,
                )} por ${Formatters.userMention(msg.author.id)}`,
                files: [media],
              })
              .then((mediaMsg) => {
                BulkEmoji(mediaMsg, ['✅', '❌'])
          ...
          msg.guild.channels.cache
            .get(mediaChannel)
            .send({
              content: `Link(s) enviado(s) em ${Formatters.channelMention(
                generalChannel,
              )} por ${Formatters.userMention(
                msg.author.id,
